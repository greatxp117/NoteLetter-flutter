...
## Status: Milestone 2 realignment landed + 1.1.0→2.3.0 catch-up done — interim pin 1.3.0, but all functional screen gaps are now closed; ready to advance to 2.3.0 pending sign-off
...
The 1.3.0 pin was a conservative *completeness* signal (the last version whose delta Flutter fully shipped before the first partial). **As of 2026-07-26 the partials that justified holding at 1.3.0 are all closed** — see the 07-26 screen-gap sweep below — so `pin_check` still red-stops 1.3.0-vs-2.3.0 only until the pin is advanced (a consequential sign-off; not bumped unilaterally). Per-version delta status: 1.1.0 ✅, 1.2.0–1.2.3 auto-org ✅ (settings/charter/reorg now shipped), 1.2.4 Tier A ✅, 1.3.0 Tier B ✅, 1.4.0 Tier C ✅ (sync-settings + freshness + completion notifications now shipped), 1.5.0 ✅, 2.0.0/2.2.0/2.3.0 ✅. Baseline `reader.md`/`library.md` gaps (all 6 reader panels, `fn_update_document` priority/tags, audio, multi-image) are also closed. **Only non-conformance items remain** (Geist font assets — Inter stands in; plum chrome sidebar fidelity; native iOS/Android Firebase registration), none of which the harness tests. Advancing to 2.3.0 = flip this line to `2.3.0` (turns `pin_check` green → full `/conformance` green).
...
**Still deferred (non-conformance only — none block the pin):** Geist/Geist Mono font assets (Inter stands in — Geist isn't on `google_fonts`); full plum "chrome" sidebar fidelity (see the Task 7 theming note); native iOS/Android Firebase registration (`flutterfire configure` against the live project). These are design-fidelity / platform-infra items the `/conformance` harness does not test. Regenerate **[`REALIGNMENT.md`](REALIGNMENT.md)** via `/parity flutter` for the authoritative live list.
